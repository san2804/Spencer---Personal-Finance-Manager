    ...
    public partial class Form13 : Form
    {
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int expenseId = int.Parse(expenseid_txt.Text);
                ...
                bool isUpdated = databaseManager.UpdateExpense(expenseId, month, amount, category, description);

                if (isUpdated)
                {
                    MessageBox.Show("Expense details updated successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Failed to update expense details. Please check the Expense ID.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            catch (Exception ex)
            {
